...
Одинаковы для **всех шести** вкладок (AgentRouter / GoRouter / Tabi / XPeach / JustWoker /
SeekAi), потому что все шесть — экземпляры одного `keepalive-proxy.js`. **Кроме одной
ручки:** мульти-запрос включён только у agentrouter, а у `tabitoken.com` / `gorouter.app` /
`xpeach.codes` / `api.justwoker.icu` / `seekai.cc` `maxHedges` дефолтно `0` (список
`FLAT_RATE_HOSTS` в коде). Причина в биллинге, замер 21.08 (seekai — 24.08):
