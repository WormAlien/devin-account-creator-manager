...
что все четыре — экземпляры одного `keepalive-proxy.js`. Замерены они на **agentrouter**;
остальные три наследуют их за отсутствием собственных замеров. Цифры консервативные (один
дубль и только на 20с тишины), так что чужому шлюзу они не навредят — но если у Tabi /
GoRouter / XPeach профиль латентности окажется другим, крутить надо их
`keepalive-config-<PORT>.json` или env, а не общий `DEFAULT_CFG`.

| ручка | дефолт | смысл |
|---|---|---|
| `hedgeMs` | `20000` | шлюз молчит 20с (нет даже заголовков) → уходит **параллельный дубль**, берём того, кто ответил первым. `0` = выкл |
| `maxHedges` | `1` | сколько ПАРАЛЛЕЛЬНЫХ дублей максимум на запрос. Свой счётчик, не бюджет `maxAttempts` |
...
`⚠ не дефолт (hedgeMs, …)`: у юзера, который однажды покрутил ручки, `json` приоритетнее
кода, и обновление прокси само ему ничего не привезёт — теперь это хотя бы видно.
Контракт `/__state.defaults` закреплён тестом `H` в `routing/test-hedge.js`.
...
делает хуже», цена дубля в деньгах, диагностика «прокси vs шлюз vs VPN-туннель»:
