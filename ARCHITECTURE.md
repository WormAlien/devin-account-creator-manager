...
- **живость keepalive проверяется по HTTP, а не по bind** (`keepaliveBring()` — единственная
  ...
  поле `keepalive` ответа, а панель хеджа при мёртвом порту поднимает его сама и применяет
...
дочитали мы ответ или порвали его. Значит каждый хедж-дубль там стоит полную цену
...
| `maxHedges` | `1`, у плоскотарифных `0` | сколько ПАРАЛЛЕЛЬНЫХ дублей максимум на запрос. Свой счётчик, не бюджет `maxAttempts`. `0` = хедж выкл |
| `maxAttempts` | `3` | всего попыток на запрос (ретраи + хеджи вместе) |
| `preCommitMs` | `10000` | 10с тишины на `stream:true` → открываем SSE клиенту и держим пингами. `0` = выкл |

`maxHedges` появилась 21.08 вместе со сменой `hedgeMs` 12с → 20с. Причина: `scheduleHedge`
перевзводил себя, и на молчащем шлюзе при `maxAttempts=3` в воздухе оказывались **три
копии** одного запроса, а бюджет попыток был выеден хеджами — на транзиентную `500`
...
1. **`CFG_VERSION` (сейчас `2`) — поднимать при КАЖДОЙ смене `DEFAULT_CFG`.** Конфиг с
   ...
   жгут баланс старым агрессивным хеджем. В логе видно строкой
   `config версии 1 устарел (сейчас 2) — беру дефолты кода`.
2. **Платный хедж прибит гвоздём над конфигом.** На хостах из `FLAT_RATE_HOSTS`
   ...
   баланс. В `/__state` для этого есть `flatRate` и `paidHedgeLocked` — по ним дашборд
...
хеджа, а сдохшая попытка таймаут.
...
**Курсор-полоска** (`latBindCursor`): вертикальная линия по мыши, маркеры на среднем и
максимуме минуты, цифры уходят в подпись под графиком (`21.08, 11:18 · ответов 4 ·
среднее 15s · быстрейший 11s · худший 23s`). Захват по всей высоте, а не по точкам:
наводить мышью на 2-пиксельный кружок невозможно, поэтому нативные `<title>` с точек
убраны. Пока мышь над графиком, поллинг **не перерисовывает** карточку (`LAT_HOVER`) —
иначе полоска и цифры исчезали бы из-под курсора раз в 18 секунд; свежие данные лежат в
`LAT_DATA` и попадают в отрисовку, как только мышь уходит.
...
Три входа — кнопка «⬇ Обновить дашборд» в Настройках (ручка
`POST /__switch/api/dashboard/update-pull`), `UPDATE.bat` → `update.sh` и
`FIX.bat` → `fix.sh`. Реализация одна: **`tools/git-pull-safe.js`**. Дублировать
...
`proxy-target.json`, `fm-openai-config.json`, тумблер `frontdoor.json`), поэтому
наивный `git pull --ff-only` упирался в «local changes would be overwritten»
навсегда — а починка этого доезжает только через тот же pull.
...
- **Регресс-тест:** `node tools/test-git-pull-safe.js` — 43 проверки в
  одноразовом репо, включая H, который вырезает блок обновления из **живых**
  `update.sh` / `fix.sh` и проверяет, что на расхождении HEAD не сдвинулся.
