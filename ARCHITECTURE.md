...
что все четыре — экземпляры одного `keepalive-proxy.js`:
...
`MAX_HEDGES` / `PRE_COMMIT_MS`) → дефолты в коде (`routing/keepalive-proxy.js`, блок
`const cfg`). Файлы конфигов в `.gitignore` (per-machine runtime), поэтому **на свежей
...
json. Плейсхолдеры инпутов (`20` / `1` / `3` / `10`) держать равными дефолтам кода.
