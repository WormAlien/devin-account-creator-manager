...
| ручка | дефолт | смысл |
|---|---|---|
| `hedgeMs` | `12000` | шлюз молчит 12с → уходит **параллельный дубль**, берём того, кто ответил первым. `0` = выкл |
...
`PRE_COMMIT_MS`) → дефолты в коде (`routing/keepalive-proxy.js`, блок `const cfg`).
Файлы конфигов в `.gitignore` (per-machine runtime), поэтому **на свежей установке
