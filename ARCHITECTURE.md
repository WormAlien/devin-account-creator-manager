...
- **Плашки «где уже используется»** на карточке: `AR`/`GO`/`TB`/`XP` + статус записи
  (цвета и ярлыки те же, что у `NEWAPI_SEED_PROV`), либо одна emerald-плашка
  `— свободен`. Данные едут вместе со списком — `handleGhKeys` отдаёт
