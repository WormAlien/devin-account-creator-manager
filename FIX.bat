@echo off
rem One-click fixer: pull latest + auto-install everything missing (incl. tg-venv).
rem ASCII only (no chcp!). Finds git-bash itself.
setlocal
...
"%BASH%" -lc "cd \"$(cygpath -u '%~dp0')\" && bash fix.sh"
