// agentrouter/restore_session.js
//
// Открывает консоль agentrouter.org с сохранённой сессией.
// Вход в консоль = чек-ин (+$25). Сессия хранит GitHub-OAuth cookies.
//
// Использование:
//   node agentrouter/restore_session.js <label>
//   node agentrouter/restore_session.js                  # покажет список доступных сессий
...
async function main() {
    ...
    if (!label) {
        ...
        console.log('\nИспользование: node agentrouter/restore_session.js <label>');
        process.exit(0);
    }
    ...
    console.log('✅ Браузер открыт с сессией (вход = чек-ин +$25). Закрой вручную когда закончишь.');
}
