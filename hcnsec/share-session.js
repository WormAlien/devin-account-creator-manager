// hcnsec/share-session.js
//
// Снимает «живую» сессию аккаунта HCNsec (cookies api.hcnsec.cn + localStorage) в
// Playwright storageState-формате и сохраняет в hcnsec/sessions/<label>.json.
//
// 🪤 GitHub-кук здесь не бывает: у hcnsec входа через GitHub нет вовсе (замер
// `/api/status` 2026-08-31: github_oauth=false), вход только email + пароль. Поэтому
// снимок — это ровно сессия панели, и папки `gh-sessions/` у этого шлюза нет.
//
// Использование:
//   node hcnsec/share-session.js <label>
...
async function main() {
  ...
  try {
    // Даём сайту время пораздавать/освежить куки в профиле.
