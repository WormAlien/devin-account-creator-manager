// freemodel-openai-proxy.js — Anthropic → OpenAI прокси для FreeModel
//
// Аналог fuergaosi233/claude-code-proxy, но на чистом Node (без Python/FastAPI)
// и заточен под наш стек. Claude Code шлёт Anthropic-формат (/v1/messages),
// прокси конвертирует в OpenAI chat/completions и отправляет на
// https://api.freemodel.dev/v1 — OpenAI-сторона FreeModel (gpt-5.5, gpt-5.6-*,
// codex). ВАЖНО: cc.freemodel.dev — только claude-модели (anthropic endpoint),
// gpt-модели живут на api.freemodel.dev, ключ тот же fe_oa_.
//
// Слушает :20130. Ключ берётся (по приоритету):
//   1. из заголовка клиента (x-api-key / Authorization: Bearer) — его пишет
//      transparent-proxy в settings.json при переключении на fm_openai;
//   2. из ~/.claude/fm-active-key.txt (режим API Helper);
//   3. из env OPENAI_API_KEY.
//
// Маппинг моделей (claude-* → gpt-*) — routing/fm-openai-config.json:
//   { "BIG_MODEL": "gpt-5.5", "MIDDLE_MODEL": "gpt-5.5", "SMALL_MODEL": "gpt-5.5" }
// opus → BIG, sonnet → MIDDLE, haiku → SMALL. Не-claude модели (gpt-*, o1-*)
// проходят как есть. Конфиг перечитывается на лету (по mtime).
//
// Endpoints:
//   POST /v1/messages               — основной (stream и нет)
//   POST /v1/messages/count_tokens  — грубая оценка (символы/4)
//   GET  /v1/models                 — passthrough на upstream
//   GET  /health, GET /__fmoai/api/status — статус/статистика
...
const LISTEN_PORT = 20130;
const UPSTREAM_BASE = process.env.FM_OPENAI_BASE_URL || 'https://api.freemodel.dev/v1';
const FM_ACTIVE_KEY_FILE = path.join(os.homedir(), '.claude', 'fm-active-key.txt');
const CONFIG_FILE = path.join(__dirname, 'fm-openai-config.json');
...
const DEFAULT_CONFIG = {
    BIG_MODEL: 'gpt-5.5',      // claude-*opus*  → сюда
    MIDDLE_MODEL: 'gpt-5.5',   // claude-*sonnet* → сюда
    SMALL_MODEL: 'gpt-5.5',    // claude-*haiku* → сюда
};

const configCache = { data: { ...DEFAULT_CONFIG }, mtime: 0 };
...
function getConfig() {
    try {
        ...
        if (st.mtimeMs !== configCache.mtime) {
            ...
            configCache.data = { ...DEFAULT_CONFIG, ...raw };
            configCache.mtime = st.mtimeMs;
            logLine(`config reloaded: big=${configCache.data.BIG_MODEL} middle=${configCache.data.MIDDLE_MODEL} small=${configCache.data.SMALL_MODEL}`);
        }
...
function mapModel(claudeModel) {
    ...
    if (m.includes('opus')) return cfg.BIG_MODEL;
    if (m.includes('sonnet')) return cfg.MIDDLE_MODEL;
    if (m.includes('haiku')) return cfg.SMALL_MODEL;
...
function resolveKey(req) {
    ...
    try {
        const k = fs.readFileSync(FM_ACTIVE_KEY_FILE, 'utf8').trim();
        if (k) return k;
    } catch {}
...
function contentPartsFromClaude(blocks) {
    // user-контент: text + image → OpenAI parts
    const parts = [];
...
function convertClaudeToOpenAI(claudeReq) {
    ...
    for (const msg of claudeReq.messages || []) {
        ...
        if (msg.role === 'user') {
            // tool_result-блоки → отдельные role:tool сообщения (идут первыми,
            // OpenAI требует их сразу после assistant с tool_calls)
...
function handleStreaming(clientRes, upstreamRes, claudeReq) {
    ...
    // Состояние блоков: текст — один блок, каждый tool_call (по openai index) — свой
    let nextBlockIndex = 0;
    let textBlockIndex = null;       // индекс открытого text-блока
    const toolBlocks = new Map();    // openai tool index → { claudeIndex, id, name, started }
    ...
    function processChunk(chunk) {
        ...
        for (const tc of delta.tool_calls || []) {
            ...
            if (!tb) {
                closeTextBlock(); // Anthropic-клиенты ждут text-блок закрытым до tool_use
                tb = {
...
function handleMessages(req, res, body) {
    ...
    if (!apiKey) return claudeError(res, 401, 'Нет ключа: активируй аккаунт FreeModel (fm-active-key.txt пуст)', 'authentication_error');
    ...
    // клиент отвалился — рубим upstream, не жжём квоту.
    // ВАЖНО: слушаем close на res, не на req — у IncomingMessage close
    // прилетает сразу после приёма тела (Node ≥15), а не при обрыве клиента.
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && (url === '/health' || url === '/__fmoai/api/status')) {
        return writeJSON(res, 200, {
            ok: true, upstream: UPSTREAM_BASE, port: LISTEN_PORT,
            mapping: getConfig(), stats,
            keySource: fs.existsSync(FM_ACTIVE_KEY_FILE) ? 'fm-active-key.txt' : 'header/env',
        });
    }
...
server.listen(LISTEN_PORT, '127.0.0.1', () => {
    const cfg = getConfig();
    console.log(`[FM OpenAI Proxy] :${LISTEN_PORT} → ${UPSTREAM_BASE}`);
    console.log(`  mapping: opus→${cfg.BIG_MODEL}, sonnet→${cfg.MIDDLE_MODEL}, haiku→${cfg.SMALL_MODEL}`);
    console.log(`  key: client header → fm-active-key.txt → env OPENAI_API_KEY`);
    console.log(`  status: http://localhost:${LISTEN_PORT}/__fmoai/api/status`);
});
