...
# Держим окно открытым только при двойном клике на DASHBOARD.command — там иначе
# Terminal закроется вместе с текстом ошибки. В обычном терминале пауза вредна:
# `read` съедает следующую строку вставленного блока команд, и она молча не
# выполняется (поймано живьём: пробник баланса «не запустился» — его строку
# прочитал этот read вместо Enter).
