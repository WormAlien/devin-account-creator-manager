// seekai/open-session.js
//
// Открывает видимый Chromium с ПЕРСОНАЛЬНЫМ ПРОФИЛЕМ аккаунта (полный профиль
// на диск: история, куки, localStorage, сессии GitHub + seekai).
//
// Панель — тот же New API, что у GoRouter/Tabi/JustWoker, поэтому весь механизм
// (реф-код в localStorage, GitHub-OAuth, кука ЛК, ошибки сайта) взят у justwoker
// без изменений. Отличаются только адреса и, самое важное, реф-код.
//
// Чем seekai отличается от JustWoker (замер `/api/status` 2026-08-24):
//   • ограничения по возрасту GitHub-аккаунта панель НЕ заявляет
//     (`github_minimum_account_age_days` в ответе нет) — свежий гитхаб может пройти;
//   • есть и обычная регистрация паролем (`password_register_enabled: true`);
//   • 🪤 включены `turnstile_check` и `email_verification` — капча Cloudflare на
//     форме и подтверждение почты. Именно из-за них авто-заведения (⚡) у вкладки
//     нет: сценарий без человека тут не гарантирован.
//
// Сценарий:
//   1. В дашборде добавляешь аккаунт (email, ключ можно оставить пустым), жмёшь
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем seekai/profiles/<label>/ (на аккаунт).
//   3. Ключа у аккаунта ещё нет → открывается РЕГИСТРАЦИЯ по рефке владельца.
//      Ключ уже вписан → открывается страница баланса (wallet).
//   4. В консоли SeekAi возьми API-ключ и вставь его в аккаунт кнопкой 🔑
//      на дашборде (или впиши сразу при добавлении).
//   5. Профиль сохраняется автоматически — при следующих открытиях GitHub и
//      SeekAi уже залогинены.
//
// Использование:
//   node seekai/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка seekai/profiles/<label>/)
...
const REGISTER_URL = require("../routing/lib/ref-codes.js").url("seekai");
// Ключ уже вписан → сразу баланс, а не логин. Роут `/wallet` в бандле панели есть
// (проверено 2026-08-24 по `static/js/index.*.js`: `"/console/topup"===a` → `/wallet`).
const CONSOLE_URL = 'https://seekai.cc/wallet';
// Корень нужен для прогрева перед регистрацией (см. openRegisterViaRef).
const ROOT_URL = 'https://seekai.cc/';
...
const ghCapture = require('../routing/lib/gh-live-capture.js').makeCapture({
  label,
  moduleDir: __dirname,
  poolFile: path.join(__dirname, '..', 'routing', 'seekai-sessions.json'),
});
...
//   share-код друга      → аккаунт seekai уже создан, GitHub/seekai сразу залогинены;
//   seed:'github'        → только GitHub-куки, аккаунта seekai ещё НЕТ (см. seededGithub).
...
// (поймано на ar-аккаунтах 2026-08-17, у gorouter/tabi/seekai тот же NewAPI-фронт).
...
// Ответы seekai (NewAPI) на неудачный GitHub-вход. Раньше скрипт знал только
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ seekai закрыл регистрацию новых аккаунтов (ответ сайта) — этот аккаунт создать нельзя.',
  },
  {
    // Универсальный ответ new-api про «аккаунт слишком свежий». У SeekAi лимит
    // возраста в `/api/status` НЕ заявлен (в отличие от JustWoker с его 365 днями),
    // поэтому цифру в тексте не обещаем — печатаем то, что сказал сайт.
    ...
    msg: '⚠️  похоже, сайт отверг GitHub-аккаунт ПО ВОЗРАСТУ (ответ про «дни с регистрации»). У SeekAi порог в настройках панели не объявлен — если повторяется, нужен более старый гитхаб.',
  },
  {
    code: 'git_token',
...
// GitHub-вход прошёл и мы внутри seekai (консоль/дашборд). Тогда профиль уже
...
async function main() {
  ...
  try {
    if (appliedSession && !seededGithub) {
      ...
      console.log('✅ Импортированная сессия применена (GitHub/seekai уже залогинены).');
    ...
    if (wantRegister) {
      ...
      console.log('   затем возьми ключ в консоли SeekAi и вставь его кнопкой 🔑 в дашборде.');

      const res = await waitForLogin(page, context);
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на seekai закрыта администратором — новый аккаунт не создать.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (GitHub/seekai уже залогинены, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await ghCapture.holdOpen(context); // держим открытым, закрытие — вручную
      return;
    }
    ...
    if (!loggedInEarly) console.log('   затем возьми ключ в консоли SeekAi и вставь его кнопкой 🔑 в дашборде.');
