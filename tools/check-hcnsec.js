...
// `['hcnsec', fn]` в объект `{ tab, fn, mark }`, и построчный регексп перестал находить
...
// для записи без стрелки (`fn: loadHnSessionsLight`) — иначе разбор упал бы на
...
section('routing/proxy-dashboard.html · вкладка');
{
    ...
    const navJob = navJobs.find((j) => j.tab === 'hcnsec');
    const navWhy = navJobs.length < 15 ? `литерал NAV_COUNT_JOBS не разобран (записей: ${navJobs.length})`
        : !navJob ? 'записи hcnsec в списке нет — цифра у вкладки на boot не появится'
        : !/\bloadHnSessionsLight\b/.test(navJob.fn) ? `запись есть, но зовёт не loadHnSessionsLight: ${navJob.fn}`
        : '';
    check(!navWhy, `NAV_COUNT_JOBS: счётчик в сайдбаре считает hcnsec на boot${navWhy ? ' — ' + navWhy : ''}`);
...
if (!openjs) {
    ...
    // 🪤 Вкладка обязана открываться ВСЕГДА, когда ящик привязан: снимок только меняет,
    // куда она приедет. Первая версия при отсутствии снимка не открывала её вовсе — и
    // владелец получал одну вкладку вместо двух, хотя просил именно две.
    check(/if \(!OL_EMAIL\) return;/.test(openjs) && !/if \(!OL_SNAPSHOT \|\| !fs\.existsSync\(OL_SNAPSHOT\)\) \{\s*\n\s*console\.log\([^\n]*negde/.test(openjs),
        'вторая вкладка открывается при любом состоянии снимка — единственное условие это привязанный ящик');
    check(/HN_OL_PASS/.test(openjs) && /HN_OL_PASS: String\(olBox\.password/.test(proxy),
        'пароль ящика уезжает в скрипт: без снимка вторая вкладка приедет на форму Microsoft, пустая она бесполезна');
    check(/OL_EMAIL_SEL[\s\S]{0,200}usernameEntry/.test(openjs) && /OL_PASS_SEL[\s\S]{0,200}passwordEntry/.test(openjs),
        'селекторы формы Microsoft живые (usernameEntry/passwordEntry), а не снятые loginfmt/i0116');
    check(/один, сразу после адреса/.test(openjs) || /клик ровно/.test(openjs),
        'в коде зафиксировано: «Далее» и «Вход» — одна кнопка, клик ровно один');
}
