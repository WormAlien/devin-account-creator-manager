 ...
 *
 * 🪤 Эта дырка БЫЛА, и проверка на неё стояла красной намеренно — а 05.09 закрылась, так
 * что теперь она зелёная. Разбор оставлен: вывод дочерних скриптов
 * (`outlook/open-session.js`, `outlook/read-code.js`) уезжает в лог дашборда ЦЕЛИКОМ, а
 * сами они печатают адрес ящика человеку в консоль — так и надо, их запускают руками.
 * Значит маску обязана ставить сторона, которая релеит строку: `logLine(…${String(d)}…)`
 * и хвост stderr читалки прогоняются через `olMaskInText`. Вывод общий и переносимый:
 * маску ставит тот, кто релеит чужой вывод, а не автор вывода. Пока её не было, полный
 * адрес купленного ящика лежал в логе, который уезжает в скриншоты README.
 *
