...
// `['seekai', fn]` в объект `{ tab, fn, mark }`, и построчный регексп перестал находить
...
// для записи без стрелки (`fn: loadSkSessionsLight`) — иначе разбор упал бы на
...
section('routing/transparent-proxy.js · реестры');
{
    ...
    // Набор тегов в regexp роутов авторотации растёт (25.08 добавился `ts`) — проверяем
    // наличие 'sk' внутри группы, а не её точный вид: иначе каждый новый шлюз ронял бы
    // этот регресс с сообщением «sk потерялся», хотя терялся только шаблон здесь.
    check(/\(ar\|go\|tb\|xp\|jw\|sk[a-z|]*\)/.test(proxy), 'роуты авторотации принимают тег sk');
}
...
section('routing/proxy-dashboard.html · вкладка');
{
    ...
    // 'sk', а не точный литерал: жёсткое сравнение падало на каждом новом шлюзе и
    // сообщало «sk потерялся», хотя терялся только шаблон в этой строке.
    check(/for \(const p of \[[^\]]*'sk'[^\]]*\]\)/.test(html),
        "общий цикл по вкладкам-пулам включает 'sk'");
    const navJobs = navCountJobs(html);
    const navJob = navJobs.find((j) => j.tab === 'seekai');
    const navWhy = navJobs.length < 15 ? `литерал NAV_COUNT_JOBS не разобран (записей: ${navJobs.length})`
        : !navJob ? 'записи seekai в списке нет — цифра у вкладки на boot не появится'
        : !/\bloadSkSessionsLight\b/.test(navJob.fn) ? `запись есть, но зовёт не loadSkSessionsLight: ${navJob.fn}`
        : '';
    check(!navWhy, `NAV_COUNT_JOBS: счётчик в сайдбаре считает seekai на boot${navWhy ? ' — ' + navWhy : ''}`);
