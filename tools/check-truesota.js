 ...
 * инструменты — со стороны это «модель тупит», а не поломка. Поэтому секция «статус»
 * требует opus-only тир-карту и метку непригодных моделей в UI.
 *
 * 🪤 С 2026-09-05 вкладка ЛЕГАСИ (решение владельца): кнопка перенесена в свёрнутую
 * группу «Чтим память» рядом с XPeach и SeekAi, из `DEFAULT_TABS_VISIBLE` убрана, шлюз
 * `legacy: true` в `ref-codes.js`. Причина — ровно эти две рабочие модели: шлюз узкий,
 * а не сломанный. Всё, что он ВЫПОЛНЯЕТ, регресс по-прежнему требует живым: резолв
 * реф-ссылки, `truesota/open-session.js`, keepalive :20160, опрос вотчдогом, тир-карта.
 *
...
// `['truesota', fn]` в объект `{ tab, fn, mark }`, и построчный регексп перестал находить
...
// для записи без стрелки (`fn: loadTsSessionsLight`) — иначе разбор упал бы на
...
section('routing/proxy-dashboard.html · вкладка');
{
    ...
    // ЛЕГАСИ с 2026-09-05 (решение владельца): кнопка живёт в свёрнутой группе «Чтим
    // память» рядом с XPeach и SeekAi, в дефолтном сайдбаре её нет. 🪤 Причина НЕ
    // «сломался»: шлюз рабочий, но узкий — наш системный промпт исполняют только
    // `claude-opus-5` и `claude-opus-5-thinking`, остальные 16 моделей каталога это реселл
    // Kiro с чужим префиксом, и места в сайдбаре это не оправдывает.
    // 🪤 Утверждение ПЕРЕВЁРНУТО вместе с решением, а не снято: снятая проверка не
    // заметила бы, если кнопку случайно вернут в основную группу nav.
    const memoryGroup = (/data-extra-nav="memory"[\s\S]*?\n      <\/div>/.exec(html) || [''])[0];
    check(/data-tab="truesota"/.test(memoryGroup),
        'кнопка TrueSOTA в группе «Чтим память» (data-extra-nav="memory") — легаси с 05.09, шлюз при этом рабочий');
    const listOf = (name) => (new RegExp(`^const ${name} = .+$`, 'm').exec(html) || [''])[0];
    const tabs = listOf('DEFAULT_TABS_VISIBLE');
    // Решение владельца 05.09: `truesota` из ДЕФОЛТНОГО набора убран — шлюз рабочий, но
    // узкий, и места в сайдбаре не оправдывает. 🪤 С переездом кнопки в «Чтим память»
    // одним кликом в «Настроить вкладки» его уже не вернуть: в список менеджера попадают
    // только прямые дети #main-nav (`getMainNavButtons`), а вкладка открывается изнутри
    // группы — саму группу включает глазок 🕯. Поэтому здесь проверяется обратное: в
    // списке дефолта его быть не должно, а кнопка и панель — на месте (проверено выше).
    // Утверждение перевёрнуто вместе с решением, а не снято: снятая проверка не заметила
    // бы, если вкладку вернут в дефолт случайно.
    ...
    const navJob = navJobs.find((j) => j.tab === 'truesota');
    const navWhy = navJobs.length < 15 ? `литерал NAV_COUNT_JOBS не разобран (записей: ${navJobs.length})`
        : !navJob ? 'записи truesota в списке нет — цифра у вкладки на boot не появится'
        : !/\bloadTsSessionsLight\b/.test(navJob.fn) ? `запись есть, но зовёт не loadTsSessionsLight: ${navJob.fn}`
        : '';
    check(!navWhy, `NAV_COUNT_JOBS: счётчик в сайдбаре считает truesota на boot${navWhy ? ' — ' + navWhy : ''}`);
...
// ── 7b. Легаси-раскладка (решение владельца 2026-09-05) ───────────────────────
// 🪤 Шлюз РАБОЧИЙ — легаси он не потому, что сломался, а потому что узкий: наш системный
// промпт исполняют ровно две модели каталога, `claude-opus-5` и `claude-opus-5-thinking`,
// остальные 16 обслуживаются реселлом Kiro и ставят свой префикс поверх (замер 25.08:
// «My name is Kiro»). Держать такую вкладку в сайдбаре смысла нет — она ушла в «Чтим
// память». Поэтому статус здесь двойной, и обе половины проверяются явно:
//   • в списках ДЛЯ ЧЕЛОВЕКА шлюза быть не должно (ACTIVE_PROVIDERS без truesota; своего
//     реф-кода у него и не было, настраивать в 💩 нечего);
//   • всё, что шлюз ВЫПОЛНЯЕТ, остаётся живым — резолв `url('truesota')` для
//     `truesota/open-session.js`, опрос вотчдогом и opus-only тир-карта.
section('статус · ref-codes (легаси), вотчдог и тир-карта (живые)');
{
    let rc = null;
    ...
    check(rc && Array.isArray(rc.ACTIVE_PROVIDERS) && !rc.ACTIVE_PROVIDERS.includes('truesota'),
        'truesota НЕ в ACTIVE_PROVIDERS — легаси с 05.09 (legacy: true), строки настройки рефки в UI ему не надо');
    ...
    check(/backend: 'truesota'/.test(watchdog),
        'вотчдог пулов опрашивает truesota — шлюз РАБОЧИЙ (легаси только раскладка сайдбара), его падение должно быть слышно');
...
console.log('копия вкладки GoRouter полная · тир-карта opus-only · вкладка легаси («Чтим память»), шлюз рабочий');
