...
//
// Открывает консоль xpeach.codes («🍑 Code» / XPeachCode) в видимом Chromium с
// ПЕРСОНАЛЬНЫМ ПРОФИЛЕМ на аккаунт (полный профиль на диск: куки, localStorage,
// сессия GitHub).
//
// Сценарий:
//   1. В дашборде нажимаешь 🌐 «Открыть браузер» на карточке аккаунта.
//   2. Открывается Chromium с профилем xpeach/profiles/<label>/ (на аккаунт).
//   3. Ключа у аккаунта ещё нет → открывается РЕГИСТРАЦИЯ по рефке владельца.
//      Ключ уже вписан → открывается страница баланса (/console/topup).
//   4. Профиль сохраняется автоматически — при следующих открытиях xpeach
//      уже залогинен.
//
// Использование:
//   node xpeach/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка xpeach/profiles/<label>/)
...
const CONSOLE_URL = 'https://xpeach.codes/console/topup';
// Корень нужен для прогрева перед регистрацией (см. openRegisterViaRef).
const ROOT_URL = 'https://xpeach.codes/';
...
const ghCapture = require('../routing/lib/gh-live-capture.js').makeCapture({
  label,
  moduleDir: __dirname,
  poolFile: path.join(__dirname, '..', 'routing', 'xpeach-sessions.json'),
});
...
// однажды прилетел 404 (деплой сайта / затык WAF), он оседает в кеше профиля — и
// SPA больше не поднимается НИКОГДА: на каждом открытии белый экран, хотя куки и
// логин живые (поймано на ar-аккаунтах 2026-08-17, у xpeach тот же NewAPI-фронт).
...
// Корень SPA — <div id="root"> (проверено на живой странице 2026-08-18).
...
// страницы. Поэтому на happy path хватает ОДНОЙ навигации по реф-ссылке; корень
// прогреваем только если код с первого раза не осел (см. openRegisterViaRef у tabi:
// схема «рефка → корень → рефка» давала метание страницы и рвала OAuth-state).
...
// GitHub-вход прошёл и мы внутри xpeach (консоль). Тогда профиль уже сохранён
// Chromium'ом. Все четыре маршрута есть в бандле SPA: /sign-up, /sign-in,
// /register, /login — поэтому проверяем их все, иначе «вход выполнен» напечаталось
// бы через полторы секунды после старта.
async function waitForLogin(page, context) {
  const deadline = Date.now() + LOGIN_TIMEOUT_MS;
  while (Date.now() < deadline) {
    const url = page.url();
    const cookies = await context.cookies().catch(() => []);
    const leftAuth = !/\/sign-in|\/sign-up|\/register|\/login(?!\/oauth)/.test(url);
...
async function main() {
  ...
  try {
    ...
    if (wantRegister) {
      ...
      console.log('   затем возьми ключ на /console/token и вставь его кнопкой 🔑 в дашборде.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (GitHub/xpeach уже залогинены, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await ghCapture.holdOpen(context); // держим открытым, закрытие — вручную
      return;
    }
    ...
    if (!loggedInEarly) console.log('   затем возьми ключ на /console/token и вставь его кнопкой 🔑 в дашборде.');
